...
%>
 ...
 <%
	...
	//보여줄 공지사항 가져오기
	ArrayList<Notice> noticeList = NoticeDao.selectNotice(rowPerPage, beginRow);
 %>
            ...
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="normal-table-list mg-t-30">
                        ...
                        <!-- rowPerPage dropDown -->
					 	<form action="<%=request.getContextPath() %>/notice/noticeList.jsp" style="text-align: right;">
                        ...
                        <div class="bsc-tbl-cds">
                            <table class="table table-condensed">
								...
								%>
										<tr>
											...
											<!-- 공지글 누르면 상세보기 보여줌 -->
											<td>
												<a href = "<%=request.getContextPath()%>/notice/noticeOne.jsp?noticeNo=<%=n.getNoticeNo()%>&rowPerPage=<%=rowPerPage%>&currentPage=<%=currentPage%>"><%=n.getNoticeTitle()%></a>
											</td>
							...
							<%
								...
								//디버깅
								System.out.println(lastPage + "<-- noticeList.jsp에서 lastPage");
								////페이징하기위한 범위
							...
							%>
										<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/notice/noticeList.jsp?currentPage=<%=currentPage - 1%>&rowPerPage=<%=rowPerPage%>">&lt;이전</a></li>
							<%
									}
								}
								...
								for (int i = 1; i <= 10; i += 1) {
									...
									if (request.getParameter("rowPerPage") == null) {
							%>
										<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/notice/noticeList.jsp?currentPage=<%=(pageRange*10)+i%>"><%=(pageRange*10)+i%></a></li>
							<%		
									} else {
							%>
							...
							%>
										<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/notice/noticeList.jsp?currentPage=<%=currentPage + 1%>&rowPerPage=<%=rowPerPage%>">다음&gt;</a></li>
							<%
									}
								}
							...
							<div style="text-align: right;">
								<a href="<%=request.getContextPath()%>/notice/insertNoticeForm.jsp"><button type="button" class="btn btn-secondary">공지 추가</button></a>
							</div>	
                        </div>
                    </div>
                </div>
