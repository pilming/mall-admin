...
%>
 ...
 <%
	...
	System.out.println(currentPage  + "<-- ebookList.jsp에서 currentPage ");
	...
	System.out.println(rowPerPage + "<-- ebookList.jsp에서 rowPerPage");
	
	
	//시작 행
	int beginRow = (currentPage - 1) * rowPerPage;
	System.out.println(beginRow + "<-- ebookList.jsp에서 beginRow");
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="normal-table-list mg-t-30">
                        ...
                        <!-- 정렬버튼 -->
					 	<ul class="nav">
							<li class="nav-item">
								<a class="nav-link" href="<%=request.getContextPath()%>/ebook/ebookList.jsp">전체보기</a>
							</li>
						<%
						...
						<%
							if(categoryName != null) { //선택된 카테고리가 있으면 카테고리정보도 넘겨줌
						%>
                        ...
                        <div class="bsc-tbl-cds">
							...
							<%
								...
								if(categoryName != null) { //선택된 카테고리가 있을때
									//전체 페이지
									...
									System.out.println(lastPage + "<-- ebookList.jsp에서 lastPage");
									////페이징하기위한 범위
									...
									if(currentPage > 1) { //페이지1이 아닐경우
										...
										if(request.getParameter("rowPerPage") == null) {
							%>
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/ebook/ebookList.jsp?currentPage=<%=currentPage - 1%>&categoryName=<%=categoryName%>">&lt;이전</a></li>					
							<%
						
										} else {////rowPerPage로 받은값이 있을경우 		
							%>
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/ebook/ebookList.jsp?currentPage=<%=currentPage - 1%>&rowPerPage=<%=rowPerPage%>&categoryName=<%=categoryName%>">&lt;이전</a></li>	
							<%
										}
									}
							...
							<%
									...
									if (currentPage < lastPage) {
										...
										if (request.getParameter("rowPerPage") == null) {
							%>
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/ebook/ebookList.jsp?currentPage=<%=currentPage + 1%>&categoryName=<%=categoryName%>">다음&gt;</a></li>
							<%
										} else { ////rowPerPage로 받은값이 있을경우 	
							%>
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/ebook/ebookList.jsp?currentPage=<%=currentPage + 1%>&rowPerPage=<%=rowPerPage%>&categoryName=<%=categoryName%>">다음&gt;</a></li>
							<%
										}
									}		
									...
									if(currentPage > 1) { //페이지1이 아닐경우
										...
										if(request.getParameter("rowPerPage") == null) {
							%>
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/ebook/ebookList.jsp?currentPage=<%=currentPage - 1%>">&lt;이전</a></li>
							<%
						
										} else {////rowPerPage로 받은값이 있을경우 		
							%>
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/ebook/ebookList.jsp?currentPage=<%=currentPage - 1%>&rowPerPage=<%=rowPerPage%>">&lt;이전</a></li>
							<%
										}
									}
							...
							<%
									...
									if (currentPage < lastPage) {
										...
										if (request.getParameter("rowPerPage") == null) {
							%>
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/ebook/ebookList.jsp?currentPage=<%=currentPage + 1%>">다음&gt;</a></li>
							<%
										} else { ////rowPerPage로 받은값이 있을경우 	
							%>
											<li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/ebook/ebookList.jsp?currentPage=<%=currentPage + 1%>&rowPerPage=<%=rowPerPage%>">다음&gt;</a></li>
							<%
										}
									}			
								}
							...
							<div style="text-align: right;">
								<a href="<%=request.getContextPath()%>/ebook/insertEbookForm.jsp"><button type="button" class="btn btn-secondary">Ebook추가</button></a>
							</div>	
                        </div>
                    </div>
                </div>
